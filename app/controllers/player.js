...
const addquestion=async(req,res)=>{
    if (myCache.has("songs")) {
        ...
        const normal=questions.filter(item=>item.level=='2').map(item=>item._id)
        const difficult=questions.filter(item=>item.level=='3').map(item=>item._id)
        const easy_list=randomslice(easy,5)
        const normal_list=randomslice(normal,5)
        const difficult_list=randomslice(difficult,5)
        const list_questions=[...easy_list,...normal_list,...difficult_list]
        const anwsers=await AnswerUser.create({user:user,questions:list_questions})
...
const supportquestion=async(req,res)=>{
        ...
        if (support_type=='1'){
            ...
            const question_change=await Question.findById(id_change[0],{id:"$_id",question:1,answer:1,choice:1})
            const answeruser=await AnswerUser.findByIdAndUpdate(id, { $set: { "questions.$[element]" : id_change[0] } },
            { arrayFilters: [ { "element._id":ObjectId(question_id) } ] })
...
const answer=async(req,res)=>{
    ...
    const data_user_update=answer==question.answer && time<60?{$inc:{coins:100}}:{}
    ...
    if (answer==question.answer){
        if (time<60 || support){
            ...
            if (index==listquestions.length-1){
                ...
                data={...data,'correct':false,'message':"expiried",time:time}
            }
        }
        else{
